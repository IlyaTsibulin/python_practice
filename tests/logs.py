...
def post_req(essence, data):
    essence_info = base_request.post(essence, data)
    pprint.pprint(essence_info)
    base_logger.info(essence_info)
...
@allure.feature('Add new plant')
@allure.story('Check if a new plant is added to the database')
def test_newPlant():
    plant_info = base_request.get('plants', 4)
    assert data["name"] == plant_info["name"]
